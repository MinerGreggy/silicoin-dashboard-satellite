...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'silicoin-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('silicoin').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('silicoin')[service]);
